...
public class TopologyController {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(com.sino.sdn.dci.application.controller.common.TopologyController.class);
    ...
    @GetMapping("/tree")
    public ResponseEntity getTreeTopology(Authentication authentication)
        ...
        try (final ReadTransaction tx = databroker.newReadOnlyTransaction()) {
            ...
            final Optional<NetworkTopology> readData =
                    tx.read(LogicalDatastoreType.CONFIGURATION, iid).get(TIMEOUT, TimeUnit.SECONDS);

            List<Topology> topologyList = readData.get().getTopology();
            for (Topology topology : topologyList) {
                System.out.println(topology.getTopologyId() + "======" + topology.getNode());
            }
    ...
    public ResponseEntity putTopologyOperational(@PathVariable final String topologyId, Authentication authentication)
        ...
        tx.put(LogicalDatastoreType.CONFIGURATION, iid, topology);
